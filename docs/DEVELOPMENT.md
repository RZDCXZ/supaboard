...
6. 部署 `delete-task` 与 `add-member-by-email`，保持 JWT verification 开启，并设置 `APP_ORIGIN=http://localhost:3000`。
...
pnpm exec supabase gen types --linked --lang=typescript > /private/tmp/database.remote.ts
diff -u src/types/database.ts /private/tmp/database.remote.ts
pnpm lint
pnpm typecheck
pnpm build
git status --short
...
> 实施状态（2026-07-13）：已完成。已在个人账号创建新加坡区专用免费学习项目 `supaboard-learning`，推送并核对 10 个迁移；配置本地 redirect URL、邮箱确认、GitHub Provider、仅私有 Realtime 频道和 `APP_ORIGIN`，部署保持 JWT verification 的 `delete-task` 与 `add-member-by-email`。两个邮箱用户和一个独立 GitHub 身份完成 Auth、SSR 会话、工作区、头像、私有附件签名下载、成员管理、Postgres Changes、Presence、typing Broadcast 与权限撤销 smoke test。托管环境发现私有频道会拒绝默认公开的 Postgres Changes 频道，已将客户端频道改为 `private: true`，并新增成员级 topic 授权迁移与 pgTAP 覆盖；修复后双窗口实时协作通过。远端 typegen 仅比本地迁移类型多 PostgREST 14.5 运行时元数据，公开 schema 类型一致；security advisors 无问题，performance advisors 仅有新建空项目的 INFO 项。测试工作区、三个 Auth 用户及 Storage 对象已清理，托管配置和 `.env.local` 的公开连接值保留；未部署 Next.js，也未上传 Seed。
